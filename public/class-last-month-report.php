    ...
    public function detail_about_calls($category) {
    	...
    	foreach ($allCalls as $singleCall) {
    		...
    		if($stockCat == $category) {
				...
				$roiOnInvestment = $this->roiOnInvestment($netProfitLossTillYet, $this->totalInvestmentTillYet);

				$totalAverageTimePeriod =  $this->totalAverageTimePeriod($this->totalTimePeriod, $totalCallsgivenTillYet);

				$this->totalInvestmentTillYet = $this->totalInvestmentTillYet($entryPrice, $noOfUnits);
				
							

				$annualisedROI = $this->annualisedROI($netProfitLossTillYet, $this->totalInvestmentTillYet, $totalAverageTimePeriod);
